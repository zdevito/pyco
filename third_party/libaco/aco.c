...
void aco_resume(aco_t* resume_co){
    ...
    resume_co->calling_co = aco_gtls_co;
    if (!resume_co->calling_co) {
        // initialize
        resume_co->calling_co = aco_create(NULL, 0, NULL, NULL);
    }
    aco_gtls_co = resume_co;
    acosw(resume_co->calling_co, resume_co);
    aco_gtls_co = resume_co->calling_co;
}
